...
function HeaderPage() {
    ...
    const renderHeaderContent = () => {
        return (
            <>
                ...
                <div className="header-content">
                    <h4 style={{ color: "rgba(255, 255, 255, .6)", fontSize: "1.3rem", fontWeight: "bold" }}>
                        Programming Student and Front-end Developer
                    </h4>
                </div>

                <div className="header-content">
                    <span style={{ color: "rgba(255, 255, 255, .6)", fontSize: "1.2rem" }}>From Brazil - SC</span>
                </div>
    ...
    const renderIcons = () => {
        return (
            <div data-aos="fade-left" className="header-content icons">
                <a href="https://github.com/drypzz/" target="_blank" rel="noreferrer"><BsGithub size={35} /></a>
                ...
                <a data-tooltip-id="tooltip-discord" data-tooltip-content="Copiar Discord" style={{ cursor: "pointer" }} onClick={clipDiscord}><BsDiscord size={35} /></a>
            </div>
        );
    };
    ...
    const renderInfos = () => {
        return (
            <div data-aos="fade-right" className="header-content infos">
                <p className="fadeinText">{`"${messagesDevs.toast[currentMessageIndex]["message"]}"`}</p>
                <div><span className="fadeinText">- {messagesDevs.toast[currentMessageIndex]["author"]} </span></div>
            </div>
        );
    };
    ...
    return (
        <div className="header-container">
            ...
            <Tooltip id={`tooltip-discord`} arrowColor="rgba(3, 126, 219, 0.4)" style={{ backgroundColor: "rgba(3, 126, 219, 0.4)", borderRadius: "5px" }} />
        </div>
    );
}
